...
pub fn build(b: *std.Build) void {
    ...
    const ui_app = b.addExecutable(.{
        .name = "zig-coding-agent-ui",
        .root_module = b.createModule(.{
            .root_source_file = b.path("src/frontend_main.zig"),
            .target = target,
            .optimize = optimize,
        }),
    });
    ...
    b.installArtifact(ui_app);

    const run_ui_step = b.step("run-ui", "Run the Zig Coding Agent frontend UI");
    const run_ui_cmd = b.addRunArtifact(ui_app);
    run_ui_cmd.step.dependOn(b.getInstallStep());
    run_ui_step.dependOn(&run_ui_cmd.step);
    
    const web_app = b.addExecutable(.{
    .name = "zig-coding-agent-web",
    .root_module = b.createModule(.{
        .root_source_file = b.path("src/frontend_web_main.zig"),
        .target = target,
        .optimize = optimize,
    }),
});
...
const run_web_step = b.step("run-web", "Run the Zig Coding Agent web UI");
