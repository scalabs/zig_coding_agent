...
pub fn callOpenAI(
    ...
    if (app_config.openai_api_key.len == 0) {
        return .{
            .id = null,
            .model = try allocator.dupe(u8, request.model orelse app_config.modelForProvider("openai")),
    ...
    const model_name = request.model orelse app_config.modelForProvider("openai");
