...
pub fn callBedrock(
    ...
    const runtime_base_url = try resolveRuntimeBaseUrlAlloc(allocator, app_config);
    ...
    const converse_body = renderConverseBodyAlloc(allocator, request.messages) catch |err| switch (err) {
        ...
        error.UnsupportedBedrockMessageRole => {
            return try errorResponse(
                allocator,
                model_name,
                "Bedrock scaffold supports only system, user, and assistant message roles.",
            );
        },
        else => return err,
    };
...
fn resolveRuntimeBaseUrlAlloc(
    ...
    if (app_config.bedrock_runtime_base_url.len > 0) {
        return try allocator.dupe(u8, app_config.bedrock_runtime_base_url);
    }
